...
@HiltViewModel
class ListViewModel @Inject constructor(private val interactor: TrnxInteraction): ViewModel() {

    //StateFow
    private val _state = MutableSharedFlow<NetworkResult<Transactions>>()
    val state = _state.asSharedFlow()

    fun getTrnxList(page: String){
        launchIo {
            _state.emit(NetworkResult.Loading())
            when (val response = interactor.provideTrnxUseCase().invoke(page)) {
                is Resource.Failure -> {
